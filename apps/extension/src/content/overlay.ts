...
} from './caption-language-resolver';
...
export class SubtitleOverlay {
  ...
  private resolverReasonCode: ResolverReasonCode = 'insufficient-confidence';
  private resolverDetail = 'Resolver has not run yet.';
  private manualTrackOverride = false;
  ...
  private async loadPreferences() {
    try {
      ...
      if (response.success) {
        ...
        this.isVisible = this.preferences?.subtitles?.overlayEnabled ?? true;
      }
    } catch (error) {
  ...
  private async loadSubtitles() {
    try {
      ...
      const preDecision = resolveCaptionLanguage({
      ...
      this.resolverReasonCode = preDecision.reasonCode;
      this.resolverDetail = preDecision.detail;

      if (!preDecision.shouldShowOverlay || !preDecision.selectedTrackId) {
        this.isVisible = false;
        this.applyVisibility();
        console.log('[Overlay] Overlay gated before subtitle fetch:', preDecision.reasonCode, preDecision.detail);
        return;
      }

      const selectedTrack = tracks.find((track) => track.id === preDecision.selectedTrackId) ?? null;
      if (!selectedTrack) {
        this.isVisible = false;
        this.applyVisibility();
        this.resolverReasonCode = 'insufficient-confidence';
        this.resolverDetail = 'Resolver picked a track that was unavailable by the time subtitles loaded.';
        return;
      }
      ...
      const finalDecision = resolveCaptionLanguage({
        preferences: this.preferences,
        tracks,
      ...
      this.resolverReasonCode = finalDecision.reasonCode;
      this.resolverDetail = finalDecision.detail;
      this.detectedLanguage = finalDecision.sourceLanguage;
      this.selectedTrackId = finalDecision.selectedTrackId;
      this.isVisible = finalDecision.shouldShowOverlay;
      this.applyVisibility();
      console.log('[Overlay] Resolver decision:', finalDecision);
      ...
      if (this.cues.length === 0 && this.isVisible) {
  ...
  private getPageMetadataSignals() {
  ...
  getResolverReasonCode(): string {
    return this.resolverReasonCode;
  }

  getResolverDetail(): string {
    return this.resolverDetail;
  }
