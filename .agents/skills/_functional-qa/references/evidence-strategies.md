...
11. For reviewer-facing interaction recordings, hold on the settled pre-action state long enough for a human to read it, show the actual tap or click, then linger on the first stable post-action state. Do not cut directly from "state visible" to "already dismissed."
12. If deterministic fast-forward, state injection, or a dev jump is used to reach the proof moment, the recorded state must still be semantically coherent to a reviewer. Do not ship a clip where the visible prompt, HUD, or objective implies one target while the captured review state shows another without explanation.
13. When the interaction point would otherwise be too subtle or fast, add an explicit tap or cursor indicator, or supplement the video with ordered frames that show pre-input, input, and post-input states.
