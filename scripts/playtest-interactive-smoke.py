...
class InteractivePlaytest:
    ...
    async def get_annotation_count(self, page: Page) -> int:
        """Read the annotation count from the pill toolbar or JS state."""
        # Try reading directly from React state via JS
        try:
            count = await page.evaluate("() => { const badge = document.querySelector('.playtest-pill-badge'); return badge ? parseInt(badge.textContent, 10) || 0 : 0; }")
            if count > 0:
                return count
        except Exception:
            pass
        # Try expanded pill notes
    ...
    async def run(self) -> bool:
        ...
        async with async_playwright() as pw:
            ...
            # 4. Draw a second stroke (horizontal line)
            print("\n[4/8] Drawing second stroke...", flush=True)
            # Re-activate draw tool if not already active
            pen_btn2 = page.locator(".playtest-tool[title='Draw']")
            is_active = await pen_btn2.evaluate("el => el.classList.contains('playtest-tool-active')")
            if not is_active:
                await pen_btn2.evaluate("el => el.click()")
                await asyncio.sleep(0.3)
            ...
            if await canvas.count() > 0:
                ...
                if box:
                    ...
                    count_after_stroke2 = await self.get_annotation_count(page)
                    self.record("Second draw stroke created annotation", count_after_stroke2 == 2, f"count={count_after_stroke2}")
            ...
            # After clicking Comment, pill collapses and place overlay appears
            ...
            if overlay_visible:
                # Tap on the overlay to place a comment pin
                vp = page.viewport_size
                tap_x = int(vp["width"] * 0.6)
                tap_y = int(vp["height"] * 0.5)
                await page.mouse.click(tap_x, tap_y)
                await asyncio.sleep(0.5)

                # Comment textarea should appear in expanded pill
            ...
            # Need to re-expand pill and click Comment again
            pill_toggle = page.locator(".playtest-pill-toggle")
            if await pill_toggle.count() > 0:
                await pill_toggle.evaluate("el => el.click()")
                await asyncio.sleep(0.3)
            ...
            if await comment_btn2.count() > 0:
                ...
                if await place_overlay.count() > 0:
                    vp = page.viewport_size
                    tap_x = int(vp["width"] * 0.3)
                    tap_y = int(vp["height"] * 0.7)
                    await page.mouse.click(tap_x, tap_y)
                    await asyncio.sleep(0.5)
